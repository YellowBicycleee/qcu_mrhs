...
__device__ __forceinline__ void gather_color_spinor (Float2_dst*  __restrict__ global_dst_ptr,
    ...
    Float2_dst* dst_point_element_ptr = global_dst_arr + IDX4D(t, z, y, x, Lz, Ly, half_Lx) * (Ns * n_color * m_input);
    ...
    for (int i = 0; i < m_input; i++) {
        ...
        for (int j = 0; j < Ns; j++) {
            for (int k = 0; k < n_color; k++) {
                ...
                dst_point_element_ptr[IDX3D(k, j, i, Ns, m_input)] = dst_temp;  // Nc, Ns, m_input
            }
        }
    }
}
...
__device__ __forceinline__ void scatter_color_spinor (Float2_dst**  __restrict__ global_dst_array,
    ...
    Float_src* src_point_element_ptr = global_src_ptr + IDX4D(t, z, y, x, Lz, Ly, half_lx) * (Ns * n_color * m_input);
    Float_dst* dst_point_element_ptr;
    ...
    for (int i = 0; i < m_input; i++) {
        ...
        for (int j = 0; j < Ns; j++) {
            for (int k = 0; k < n_color, k++) {
                src_temp = src_point_element_ptr[IDX3D(k, j, i, n_color, Ns)];  // Nc, Ns, m_input
                dst_temp = shiftDataType<Float2_dst, Float2_src>(src_temp);
...
__global__ void color_spinor_scatter_kernel (Float2_dst*  __restrict__ global_dst_array,
    ...
    for (int i = idx; i < vol; i += stride){
        get4DCoord(t, z, y, x, i, Lz, Ly, half_Lx);
        scatter_color_spinor<Float2_dst, Float2_src>(global_dst_ptr, global_src_array, x, y, z, t, Lx, Ly, Lz, Lt, n_color, m_input);
    }
}
