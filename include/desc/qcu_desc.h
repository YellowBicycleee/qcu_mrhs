...
struct QcuLattDesc {
    ...
    QCU_HOST_DEVICE int lattice_volume() const { return X() * Y() * Z() * T(); }
    QCU_HOST_DEVICE int half_lattice_volume() const { return lattice_volume() / 2; }
};
...
struct QcuProcDesc {  // process description, how many process in each dimension
    ...
    // volume
    QCU_HOST_DEVICE int process_volume() const { return X() * Y() * Z() * T(); }
};
}  // namespace qcu
