...
bool BiCGStabImpl<OutputPrecision, IteratePrecision>::solve_odd() {
  ...
  // r0 = b - Ax = b ----->r0，整个迭代过程不改变, 由于r0不变，初始化为b，因此直接指向b
    ...
    using OutputNormArgument =
        typename InteriorOperator::template ComplexNorm<ReduceFloat, ComputeFloat>::ComplexNormArgument;
    ...
    using OutputDotcArgument =
        typename InteriorOperator::template ComplexDotc<ReduceFloat, ComputeFloat>::DotcArgument;
    ...
    using OutputElementwiseDivArgument =
        typename InteriorOperator::template ElementwiseDiv<Complex<ReduceFloat>>::ElementwiseDivArgument;
    ...
    using OutputElementwiseMulArgument =
        typename InteriorOperator::template ElementwiseMul<Complex<ReduceFloat>>::ElementwiseMulArgument;
