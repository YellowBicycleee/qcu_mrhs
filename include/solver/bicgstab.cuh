...
class BiCGStab {
public:
  BiCGStab () : exterior_dslashParam_(nullptr) {};
  BiCGStab (DslashParam* param) : exterior_dslashParam_(param) {}
  ~BiCGStab() noexcept = default;
  void setDslashParam(DslashParam* param) { exterior_dslashParam_ = param; }
...
private:
  ...
  struct InteriorOperator {
    ...
    ComplexDotc<OutputFloat, IterateFloat> output_dotc;  // 双精度内积运算符
    ...
    ComplexNorm<OutputFloat, IterateFloat>  output_norm;    // 高精度norm2 运算符
  ...
  static constexpr int MaxOutputPrecisionFermion_ = 9;  // 输出精度的fermion个数
  ...
  void* outputBuffer_[MaxOutputPrecisionFermion_]; // 输出精度buffer // reg9 - x_o, reg10 - r_e
