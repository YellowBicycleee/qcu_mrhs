...
void single_cxpay (Complex<InputFloat>* res,
...
void array_cxpay (Complex<InputFloat>* res,
                  Complex<InputFloat>* x, 
                  Complex<InputFloat>* a, 
                  Complex<InputFloat>* y, 
                  int single_vec_len, 
                  ...
                  ) 
{
  for (int i = 0; i < num_vecs; ++i) {
    single_cxpay (res, x, a, y, single_vec_len, num_vecs, i);
  }
}
...
int main () {
  ...
  // GPU calculate
  using CxpayArgument = qcu::qcu_blas::Complex_xpay<InputFloat>::Complex_xpayArgument;
  ...
  qcu::qcu_blas::Complex_xpay<InputFloat> cxpay;
  cxpay(arg);
  CHECK_CUDA(cudaStreamSynchronize(nullptr));

  // cpu calculate
  array_cxpay(cpu_res, cpu_x, cpu_a, cpu_y, single_vec_length, num_vecs); 
