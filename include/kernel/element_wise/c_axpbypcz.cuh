...
void caxpbypcz_stride_kernel ( Complex<_Tp>* res, 
                              Complex<_Tp>*  a,   Complex<_Tp>* x, 
                              Complex<_Tp>*  b,   Complex<_Tp>* y, 
                              Complex<_Tp>*  c,   Complex<_Tp>* z,
                              ...
                              int start_idx = 0)
{
  ...
  Complex<_Tp> in_c = Complex<_Tp>(*reinterpret_cast<Float2*>(c + start_idx));
  Complex<_Tp> in_x;
  Complex<_Tp> in_y;
  Complex<_Tp> in_z;
  Float2       float2_res;
  ...
  for (int i = global_id; i < single_vec_len; i += total_thread) {
    ...
    in_z = Complex<_Tp>(*reinterpret_cast<Float2*>(z + start_idx + i * inc_idx));
    res_val = in_a * in_x + in_b * in_y + in_c * in_z;
