...
void elementwise_mul_cpu (Complex<InputFloat>* res,
...
int main () {
  ...
  // GPU calculate
  using ElementwiseMulArg = qcu::qcu_blas::ElementwiseMul<Complex<InputFloat>>::ElementwiseMulArgument;

  ElementwiseMulArg arg (
    gpu_res,
    gpu_x,
    gpu_y,
    vector_length,
  ...
  qcu::qcu_blas::ElementwiseMul<Complex<InputFloat>> elementwise_mul;
  elementwise_mul(arg);
  CHECK_CUDA(cudaStreamSynchronize(nullptr));

  // cpu calculate
  elementwise_mul_cpu(cpu_res, cpu_x, cpu_y, vector_length); 
