...
void checkNorm (void* global_mem) {
  _Float x;
                                          ...
                                          qcu::Dslash* dslash, qcu::DslashParam& param)
{
  ...
  const int single_vec_len = vol / 2 * Nd * nColor;
  ...
  typename qcu::qcu_blas::Complex_xsay<_Float>::template Complex_xsayArgument
              xsay_argument {
                ...
                single_vec_len, // int              single_vec_len,
                mInput, // int              inc_idx,
...
bool BiCGStabImpl<OutputPrecision, IteratePrecision>::solve_odd() {
  ...
  void*       r  = r0; // 初始时 r0 == r1   因为要求<r0, r> 不为0
  void*       p  = r0; // p = r0，从此outputBuffer[1]不变，不能再使用
  ...
  // void* x_o          = static_cast<Complex<OutputFloat>*>(x_e) + vol / 2 * complex_vec_len;
  ...
  DslashParam dslashParam {
    ...
    new_b_even_output_prec_,    // fermionIn_MRHS
    outputBuffer_[1],           // p_fermionOut_MRHS,
    param_.gauge,               // gauge,
  ...
  for (currentIteration_ = 0; currentIteration_ < maxIteration_; ++ currentIteration_) {
    ...
    outputDotArg.resArr    = static_cast<Complex<OutputFloat>*>(Ap_p_norm);
    ...
    output_elementwise_div_arg.y       = static_cast<Complex<OutputFloat>*>(Ap_p_norm);
    ...
    // As = As_{j} = Doe Deo * s_{j} ----> outputBuffer_[2]
    ...
    { // converge return true
      ...
      CHECK_CUDA(cudaMemcpyAsync(norm_r_array.data(), r_new_norm,
                            ...
                            cudaMemcpyDeviceToHost, stream1));
      CHECK_CUDA(cudaStreamSynchronize(stream1));
      std::printf("r_new_norm = ");
      checkNorm<OutputFloat>(r_new_norm);
      ...
      if (bool is_converged = isConverged<OutputFloat>(norm_r_array, norm_b_array, maxPrec_)) {
        CHECK_CUDA(cudaMemcpyAsync(x_e, x_new, sizeof(OutputFloat) * vol / 2 * complex_vec_len * 2,
    ...
    output_elementwise_mul_arg.y       = static_cast<Complex<OutputFloat>*>(r_r0_norm);
    ...
    output_elementwise_div_arg.res     = static_cast<Complex<OutputFloat>*>(beta_array);
    output_elementwise_div_arg.x       = static_cast<Complex<OutputFloat>*>(beta_array);
    output_elementwise_div_arg.y       = static_cast<Complex<OutputFloat>*>(r_new_r0_norm);
