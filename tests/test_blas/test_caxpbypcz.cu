...
void single_caxpbypcz (Complex<InputFloat>* res,
                       Complex<InputFloat>* a, 
                       Complex<InputFloat>* x, 
                       Complex<InputFloat>* b, 
                       Complex<InputFloat>* y, 
                       Complex<InputFloat>* c,
                       Complex<InputFloat>* z,
                       int single_vec_len, 
                       int inc_idx,
                       int start_idx
                       ) 
{
  for (int i = 0; i < single_vec_len; ++i) {
    res[i * inc_idx + start_idx] =  a[start_idx] * x[i * inc_idx + start_idx] 
                                  ...
                                  + c[start_idx] * z[i * inc_idx + start_idx];
  }
}

void array_caxpbypcz ( Complex<InputFloat>* res,
                       Complex<InputFloat>* a, 
                       Complex<InputFloat>* x, 
                       Complex<InputFloat>* b, 
                       Complex<InputFloat>* y, 
                       ...
                       ) 
{
  for (int i = 0; i < num_vecs; ++i) {
    single_caxpbypcz (res, a, x, b, y, c, z, single_vec_len, num_vecs, i);
  }
}

bool check_caxpbypcz (Complex<InputFloat> * cpu_res, Complex<InputFloat> * gpu_res, int N) {
  for (int i = 0; i < N; ++i) {
...
int main () {
  ...
  Complex<InputFloat>* cpu_c     = new Complex<InputFloat>[num_vecs];
  ...
  Complex<InputFloat>* cpu_z     = new Complex<InputFloat>[vector_length];
  ...
  CHECK_CUDA(cudaMalloc ((void**)&gpu_z,   sizeof(Complex<InputFloat>) * vector_length));
  ...
  CHECK_CUDA(cudaMalloc ((void**)&gpu_c,   sizeof(Complex<InputFloat>) * num_vecs));
  ...
  CHECK_CUDA(cudaMemcpy(gpu_z, cpu_z, sizeof(Complex<InputFloat>) * vector_length, cudaMemcpyHostToDevice));
  ...
  CHECK_CUDA(cudaMemcpy(gpu_c, cpu_c, sizeof(Complex<InputFloat>) * num_vecs,      cudaMemcpyHostToDevice));
  // GPU calculate
  using CaxpbypczArgument = qcu::qcu_blas::Complex_axpbypcz<InputFloat>::Complex_axpbypczArgument;
  CaxpbypczArgument arg (
    gpu_res,
    gpu_a,
    gpu_x,
    gpu_b,
  ...
  qcu::qcu_blas::Complex_axpbypcz<InputFloat> caxpbypcz;
  caxpbypcz(arg);
  CHECK_CUDA(cudaStreamSynchronize(nullptr));

  // cpu calculate
  array_caxpbypcz(cpu_res, cpu_a, cpu_x, cpu_b, cpu_y, cpu_c, cpu_z, single_vec_length, num_vecs); 

  CHECK_CUDA(cudaMemcpy(h_gpu_res, gpu_res, sizeof(Complex<InputFloat>) * vector_length, cudaMemcpyDeviceToHost));
  cout << check_caxpbypcz(cpu_res, h_gpu_res, vector_length) << endl;
