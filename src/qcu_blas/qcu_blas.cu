...
void ReductionNorm2<OutputFloat, InputFloat>::operator()(ReductionParam param) {
  ...
  for (int i = 0; i < param.m_rhs; ++i) {
    ...
    device::reduction::strideNorm_1_kernel 
                        <qcu::devide::operation::AddOp, OutputFloat, InputFloat> 
    ...
    // second step
    device::reduction::reduceSumStep2_kernel <qcu::devide::operation::AddOp, OutputFloat> 
                          <<<1, blocks_per_grid>>>
...
void ReductionInnerprod<OutputFloat, InputFloat>::operator()(ReductionParam param) {
  ...
  for (int i = 0; i < param.m_rhs; ++i) {
    ...
    device::reduction::reduceSumStep2_kernel 
                          <qcu::devide::operation::AddOp, Complex<OutputFloat> > 
                          <<<1, blocks_per_grid>>>
