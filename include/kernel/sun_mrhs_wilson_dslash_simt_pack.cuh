...
>
QCU_DEVICE
void single_point_wilson_dslash_xdim_forward_ghost_pack(
    ...
    bool dagger_flag, int n_color, int m_rhs, int coord_1dim)
{
    ...
    QcuLattDesc sub_space_half_desc {1, latt_desc.Y() >> 1, latt_desc.Z(), latt_desc.T()};
    ...
    coord.at(Y_DIM) = 2 * sub_latt_coord.Y() + (cb_xzt == parity);
...
>
QCU_DEVICE
void single_point_wilson_dslash_xdim_backward_ghost_pack(
    FloatType_* __restrict__ temp_out,
    FloatType_* __restrict__ in,
    QcuLattDesc latt_desc, int ghost_dim , int parity,
    bool dagger_flag, int n_color, int m_rhs, int coord_1dim)
    ...
    int parity, bool dagger_flag, int n_color, int m_rhs)
{
    ...
    if (ghost_dim == X_DIM) {
        for (int idx = block_id; idx < half_vol; idx += grid_size) {
            single_point_wilson_dslash_xdim_backward_ghost_pack<FloatType_, BlockShape_, WarpShape_>(
    ...
    int parity, bool dagger_flag, int n_color, int m_rhs)
{
    ...
    if (ghost_dim == X_DIM) {
        for (int idx = block_id; idx < half_vol; idx += grid_size) {
            single_point_wilson_dslash_xdim_forward_ghost_pack<FloatType_, BlockShape_, WarpShape_>(
