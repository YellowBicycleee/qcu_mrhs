...
void ComplexDotc<OutputFloat, InputFloat>::operator()(DotcArgument arg) {
    ...
    if constexpr (std::is_same_v<InputFloat, double>) {
        ...
        for (int i = 0; i < stride; ++i) {
            QCU_CHECK_CUBLAS (
                cublasCdotc(cublas_handle, arg.single_vec_len,
                    reinterpret_cast<const cuComplex*>(arg.input1) + i, stride,
                    reinterpret_cast<const cuComplex*>(arg.input2) + i, stride,
                    reinterpret_cast<cuComplex*>(arg.resArr) + i
                )
            );
        }
    }
